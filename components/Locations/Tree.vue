<template>
  <ul :class="`list-none border-0 ${depth === 0 ? 'border-t ml-0' : 'ml-4 '}`">
    <li v-for="location in locations" class="">
      <div
        class="px-3 py-2 flex flex-row justify-between content-center items-center border-0 border-b border-r border-l border-t-2"
        style="margin-top: -1px;">
        <div :class="`grow leading-8 ${location.locations && location.locations.length > 0 ? 'cursor-pointer' : ''}`"
          @click="open(location)">
          <UTooltip :text="`View location ${location.name}`">
            <UButton :to="`/locations/${uuidb64(location.id)}`" variant="ghost">
              <strong class="text-lg">{{ location.name }}</strong>
              <UBadge color="teal" class="ml-3">{{ location.locations ? location.locations.length : '0' }} <span
                  class="hidden md:inline ml-1"> location{{ location.locations && location.locations.length > 1 ? 's' : ''
                  }}</span></UBadge>
              <UBadge color="yellow" class="ml-3">
                {{ partsCount(location) }}<span class="hidden md:inline ml-1"> part{{ partsCount(location) > 1 ? 's' : ''
                }} </span>
              </UBadge>
            </UButton>
          </UTooltip>
        ...
        <div class="shrink flex items-center content-center">
          <UPopover class="block md:hidden">
            ...
            <template #panel>
              <div class="p-4">
                <UButton variant="solid" icon="i-heroicons-outline-eye" :to="`/locations/${uuidb64(location.id)}`" />
                <UButton class="ml-2" variant="solid" icon="i-heroicons-outline-folder-plus"
                  @click="createModal = location.id" />
                <UButton class="ml-2" label="" icon="i-heroicons-outline-qr-code" @click="qrModal = location.id" />
                <UButton class="ml-3" variant="solid" color="red" label="" icon="i-heroicons-outline-trash"
          ...
          <div class="hidden md:block">
            ...
            <UTooltip :text="`Delete location ${location.name}`">
              <UButton class="ml-4" variant="solid" color="red" label="" icon="i-heroicons-outline-trash"
                @click="remove(location)" />
            </UTooltip>
          </div>
        </div>
        <div class="shrink h-5" v-if="location.locations && location.locations.length > 0">
          <UIcon v-if="opened[location.id]" name="i-heroicons-chevron-down-20-solid"
            class="shrink w-5 h-5 ml-2 leading-8 cursor-pointer" @click="open(location)" />
          <UIcon v-else name="i-heroicons-chevron-right-20-solid" class="shrink w-5 h-5 ml-2 leading-8 cursor-pointer"
            @click="open(location)" />
        </div>
...
const partsCount = (location: Location) => {
  ...
  return location.location_parts ? location.location_parts.length : 0
  // }
...
const remove = async (location: Location) => {
  ...
  if (location.location_parts.length > 0) {
  ...
  if (error) {
    toast.add({
      id: 'location_delete_error',
      title: 'Could not delete this location.',
