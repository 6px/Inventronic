    ...
    <li v-for="location in locations" class="">
      <div class="px-3 py-2 flex flex-row justify-between content-center items-center border-0 border-b border-r border-l">
        <div :class="`grow leading-8 ${location.Locations.length > 0 ? 'cursor-pointer' : ''}`" @click="open">
          ...
          <UBadge color="yellow" class="ml-3">{{ location.Parts ? location.Parts.length : '0' }} part{{ location.Parts.length > 1 ? 's' : '' }}</UBadge>
        </div>
        <div class="shrink flex items-center content-center">
          <UButton variant="solid" label="Edit" :to="`/locations/${location.id}`" />
          <UButton class="ml-2" label="" icon="i-heroicons-outline-qr-code" @click="qrModal=true" />
        </div>
      ...
      <LocationsQRCodeModal :open="qrModal" :location="location" @close="qrModal=false" />
